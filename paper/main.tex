...
exploitable function of \emph{any} document-intrinsic property we measured. A
QuALITY control confirms the structure axis does no harm on unstructured prose
(structure-routing rate $0$).
\end{abstract}
...
cheap, document-intrinsic route} on three independent grounds: the learned ceiling
on the surface features fails, the causally-motivated locality features fail, and
even a label-derived (near-oracle) target diagnostic is uncorrelated with the
optimum---so the failure is pinned to the target, not to a particular feature or
to label noise. It does not generalize to other corpora or to \emph{query-aware /
learned multi-signal} predictors, which we did not test. \textbf{Free-tier nondeterminism.} Clustering and LLM steps are not
