...
oracle leaves $\approx 0.10$ coverage above every predictor we tried. Our
diagnostic narrows where it could come from: since the optimum is uncorrelated
with \emph{document-intrinsic} evidence geometry (E2), the residual signal is most
likely \emph{query-aware / retrieval-dependent}---set by the question
distribution and retrieval dynamics rather than the document alone. That is the
\emph{learned, query-aware} territory of \cite{mog2024,hichunk2025}, which abandons
the cheap training-free, document-intrinsic differentiator we set out to test and
is a substantially larger project. We refute exactly that intended
contribution---cheap, closed-form, training-free, document-intrinsic leaf
sizing---and point future work at the query-aware route.
...
results are arm-vs-arm. \textbf{Scale / corpus.} The primary results are $N{=}50$ on QASPER; CIs are wide
(std $\approx 0.04$). The negative conclusion is robust \emph{for the cheap,
